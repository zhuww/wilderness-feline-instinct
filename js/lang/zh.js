/* ==========================================================================
   Wilderness Feline Instinct — lang/zh.js
   中文基准字典：全部 key → 中文
   （其余语言由翻译工程师在 en.js / fr.js / es.js / de.js 中填充；
     缺失时 Game.i18n.t 自动回退到本文件的中文）
   ========================================================================== */
Game.i18n = Game.i18n || {};
...
Game.i18n.dicts.zh = {
  ...
  /* ============================================================ ui. HUD/面板/按钮 */
  'ui.hud.time': '{icon} {time} · 第 {day} 天',           /* 插值: {icon} {time} {day} */
  ...
  'ui.craft.locked': '🔒 未解锁',
  'ui.craft.needSkill': '需要技能：{skill}',               /* 插值: {skill} */
  ...
  'ui.friends.yourPets': '🐾 你的宠物（{n}）',             /* 插值: {n} */
  'ui.friends.noPets': '还没有宠物——把流浪猫喂到 60 ♥，然后收养它！',
  'ui.friends.strays': '🐈 流浪猫（{n}）',                 /* 插值: {n} */
  'ui.friends.unknown': '还有 {n} 只流浪猫在荒野中游荡——跟着粉色气味找它们。', /* 插值: {n} */
  'ui.friends.none': '附近还没有猫。按 E 嗅探，跟着粉色气味流寻找。',
  'ui.friends.status.adopted': '好友 ❤️',
  'ui.friends.status.adoptable': '可收养——走近按 F！',
  'ui.friends.status.approaching': '{n}/60 ♥ 可收养',      /* 插值: {n} */
  ...
  'ui.growth.skillPoints': '技能点：{n}',                   /* 插值: {n} */
  'ui.growth.xp': '{xp} / {need} 经验',                    /* 插值: {xp} {need} */
  'ui.growth.bonus': '等级加成：最大生命 +{hp} · 最大体力 +{st} · 最大心情 +{mood} · 体力回复 +{regen}%', /* 插值: {hp} {st} {mood} {regen} */
  'ui.growth.crit': '心情暴击率：<b class="text-amber-300">{pct}%</b>（心情越好暴击越高，双倍伤害）', /* 插值: {pct}（含 HTML 样式，译文须保留 <b> 标签） */
  'ui.growth.notes': '技能点只在升级时获得——每升 1 级 +1 点，请谨慎规划加点路线；猎手本能 / 飞扑袭杀 / 厚实毛皮 / 活力充盈 / 灵动闪避 / 能工巧匠可重复加点。',
  'ui.growth.skillTree': '📖 技能树（已投入 {n} 点）',      /* 插值: {n} */
  ...
  'feature.prompt.gate': '前往{name}',                     /* 插值: {name} */
  ...
  'feature.beacon': '📍 {name} 信标',                      /* 插值: {name}（洞穴/泉水） */
  ...
  'log.craft.needSkill': '🔒 需要技能【{skill}】才能合成。',   /* 插值: {skill} */
  'log.craft.done': '🔨 合成了 {name}！',                     /* 插值: {name} */
  ...
  'log.zone.enter': '⛩ 你进入了【{name}】！',                 /* 插值: {name} */
  'log.death': '☠️ 你精疲力竭倒下了……在黎明中醒来。',
  'log.stumble': '🐾 你踉跄了一下，站稳了脚跟。',
  'log.pounce.water': '💦 你差点落水，扑腾着跳回岸上！（毛打湿了）',
  'log.groom': '✨ 你梳理了毛发，神清气爽！',
  'log.catch': '🐾 抓到一只{name}！',                          /* 插值: {name} */
  'log.fish.run': '🎣 三文鱼洄游中随手捞到一条！',
  'log.fish.none': '🐟 岸边没有鱼……口渴的话去找清泉吧（青色气味）。',
  'log.combat.hit': '⚔️ 你击中{name}，造成 {dmg} 伤害{crit}！', /* 插值: {name} {dmg} {crit} */
  'log.combat.kill': '💀 {name}倒下了。',                      /* 插值: {name} */
  'log.crit.bang': '暴击！',                                   /* 暴击后缀（流浪狗命中用） */
  'log.crit.wrap': '（暴击！）',                                /* 暴击后缀（扑击命中用） */
  'log.dodge': '💨 你灵巧地闪开了攻击！',
  'log.damage': '💔 你受到 {n} 点伤害！',                      /* 插值: {n} */
  'log.footsteps': '👂 附近传来脚步声……',
  'log.pred.alert': '⚠️ 一只{name}发现了你！',                 /* 插值: {name} */
  'log.poison.venom': '💚 毒液溅到你身上！（-{dmg} 生命，中毒！）', /* 插值: {dmg} */
  'log.poison.tick': '💚 毒素发作！-{n} 生命',                  /* 插值: {n} */
  'log.poison.gone': '🌿 毒素消退，你恢复了。',
  'log.level.up': '🎉 升级！你现在是 {level} 级！（+1 技能点）', /* 插值: {level} */
  'log.skill.point': '📌 获得 {n} 技能点！（当前 {points}）',   /* 插值: {n} {points} */
  'log.skill.none': '📖 没有这个技能！',
  'log.skill.maxed': '📖 {name} 已经满级（Lv.{max}）！',        /* 插值: {name} {max} */
  'log.skill.noPoint': '📌 技能点不足——只有升级才能获得技能点。',
  'log.skill.learned': '⭐ 习得技能：{name} Lv.{lv}/{max}！（-1 技能点）', /* 插值: {name} {lv} {max} */
  'log.skill.book': '📖 发现技能书：{name}！（在行囊中阅读）',   /* 插值: {name} */
  'log.skill.readBook': '📖 阅读旧技能书：+40 经验！（技能点只在升级时获得）',
  'log.equip.off': '⬇️ 摘下了{name}（仍在行囊中）。',           /* 插值: {name} */
  'log.equip.on': '⬆️ 穿上了{name}！',                         /* 插值: {name} */
  'log.zoomies': '😵‍💫 猫薄荷！！疯狂跑酷！！！{name}！',        /* 插值: {name} */
  'log.item.use': '😋 使用了{name}。',                          /* 插值: {name} */
  'log.drop.jade': '💎 猴子的巢穴里掉出一块翡翠！',
  'log.drop.sapphire': '💎 鳄鱼皮里嵌着一颗蓝宝石！',
  'log.dog.bite': '🐕 流浪狗咬了你一口！（-{dmg} 生命）',       /* 插值: {dmg} */
  'log.dog.bark': '🐕 流浪狗朝你狂吠追来！',
  'log.dog.hit': '🐕 你打中了流浪狗，{crit}它夹着尾巴逃窜！',   /* 插值: {crit} */
  'log.dog.defeated': '💀 流浪狗被你赶跑了。（+筋腱 +12 经验）',
  'log.boss.boar.charge': '🐗 巨野猪向你冲来！',
  'log.boss.boar.hit': '🐗 巨野猪撞飞了你！（-{dmg} 生命）',    /* 插值: {dmg} */
  'log.boss.wolf.hit': '🐺 巨狼咬了你！（-{dmg} 生命）',        /* 插值: {dmg} */
  'log.boss.cobra.spit': '🐍 大眼镜蛇喷出一团毒液！',
  'log.boss.cobra.leap': '🐍 大眼镜蛇如箭般扑击而来！',
  'log.boss.cobra.leapHit': '🐍 眼镜蛇扑击重创了你！（-{dmg} 生命，中毒！）', /* 插值: {dmg} */
  'log.boss.cobra.spitWindup': '🐍 大眼镜蛇蜷曲身体蓄力……（要喷毒了！）',
  'log.boss.cobra.leapWindup': '🐍 大眼镜蛇停下脚步，盘卷起身体……（要扑击了！）',
  'log.boss.cobra.bite': '🐍 眼镜蛇咬中了你！（-{dmg} 生命，中毒！）', /* 插值: {dmg} */
  'log.boss.kid.shoot': '🧒 顽童用弹弓射出一颗石子！',
  'log.boss.kid.hit': '💢 你被石子砸中了！（-{dmg} 生命）',      /* 插值: {dmg} */
  'log.boss.crit': '💥 对【{name}】造成暴击！{dmg} 伤害！',      /* 插值: {name} {dmg} */
  'log.boss.defeated': '🏆 你击败了【{name}】！获得大量经验！',  /* 插值: {name} */
  'log.boss.respawn': '⚠️ 【{name}】在竞技场复活了！',           /* 插值: {name} */
  ...
  'log.feature.gem': '💎 采到一颗{name}！（60 秒后再生）',      /* 插值: {name} */
  'log.feature.trash': '🗑 你在垃圾堆里翻出了{name}！',          /* 插值: {name} */
  'log.feature.trashEmpty': '🗑 垃圾桶里空空如也……',
  'log.feature.forest': '🍂 在森林里捡到些材料。',
  'log.feature.nothing': '😺 这里没什么可以互动的……',
  'log.companion.warn': '🐈 {name}嘶叫：有捕食者靠近！',        /* 插值: {name} */
  'log.companion.gift': '🎁 {name}给你带来了{gift}！',          /* 插值: {name} {gift} */
  'log.pet': '🐾 你抚摸{name}——它满足地咕噜咕噜叫。（+{n} ♥）', /* 插值: {name} {n} */
  'log.pet.first': '😺 {name}开始亲近你——继续抚摸，或从猫菜单喂食更快成为朋友！', /* 插值: {name} */
  'log.pet.ready': '💗 {name}已经准备好成为你的朋友——从猫菜单收养它吧！', /* 插值: {name} */
  'log.feed.none': '🍽️ 你现在没有食物可以分享（三文鱼、烤鲑鱼或老鼠）。',
  'log.feed': '🍖 你把{item}分给{name}！（+{n} ♥）',            /* 插值: {item} {name} {n} */
  'log.feed.first': '😺 {name}很喜欢！继续下去它会信任你。',     /* 插值: {name} */
  'log.adopt.notReady': '💭 {name}还没准备好——继续抚摸和喂食（需要 60 ♥）。', /* 插值: {name} */
  'log.adopt.ok': '🎉 {name}现在是你朋友了！它会一直跟着你。',   /* 插值: {name} */
  'log.perk.warn': '🐈 {name}现在会提醒你周围的危险！',          /* 插值: {name} */
  'log.perk.hunt': '🐈 {name}现在会和你并肩狩猎（+伤害）！',     /* 插值: {name} */
  'log.summon.end': '🐈 {name} 战斗结束，回到你身边。',          /* 插值: {name} */
  'log.summon.cd': '📣 召唤冷却中（{n} 秒）',                   /* 插值: {n} */
  'log.summon.none': '😿 你还没有伙伴猫——先收养一只流浪猫吧！',
  'log.summon.ok': '📣 {name} 应召而来，与你并肩作战！（冷却 {n} 分钟）', /* 插值: {name} {n} */
  'log.summon.strike': '🐈 {name} 猛扑向敌人！（{dmg} 伤害）',  /* 插值: {name} {dmg} */
  ...
  'log.challenge.viper.bite': '🐍 毒蛇咬了你！（-{n} 生命）',    /* 插值: {n} */
  'log.challenge.viper.win': '🏆 你击退了毒蛇群！（+{n} 心情）',  /* 插值: {n} */
  'log.challenge.wolf.start': '🐺 狼群正在窥伺你——反击或逃跑！',
  'log.challenge.wolf.kill': '💀 你放倒了一只狼！（+18 经验）',
  'log.challenge.wolf.stagger': '🐺 狼被打得踉跄！',
  'log.challenge.wolf.bite': '🐺 狼咬了你！（-{n} 生命）',       /* 插值: {n} */
  'log.challenge.wolf.win': '🏆 你活过了狼群！（+10 心情）',
  'log.challenge.stampede.start': '🐗 野猪狂奔！躲开冲撞的野猪！',
  'log.challenge.stampede.hit': '🐗 狂奔的野猪踩了你！（-{n} 生命）', /* 插值: {n} */
  'log.challenge.stampede.win': '🏆 你躲过了野猪狂奔！（+{n} 心情）', /* 插值: {n} */
  'log.challenge.eagle.start': '🦅 一只鹰在头顶盘旋——躲开它的俯冲！',
  'log.challenge.eagle.hit': '🦅 鹰爪抓伤了你！（-{n} 生命）',    /* 插值: {n} */
  'log.challenge.eagle.miss': '🦅 鹰从你身边俯冲掠过！',
  'log.challenge.eagle.dive': '🦅 鹰开始俯冲——快离开影子！',
  'log.challenge.eagle.win': '🏆 鹰飞走了！（+{n} 心情）',       /* 插值: {n} */
  'log.challenge.fog.start': '🌫️ 浓雾弥漫——在时间耗尽前找到{name}信标逃出去！', /* 插值: {name}（洞穴/泉水） */
  'log.challenge.fog.win': '🏆 你找到了穿过浓雾的路！（+{n} 心情）', /* 插值: {n} */
